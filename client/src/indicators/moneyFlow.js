...
export const countMoneyFlowIndexes = (quotes) => {
  const highPrices = quotes.map(column => column.high)
    .slice(0, DAYS - 1 + QUOTES)
    .reverse();
  const lowPrices = quotes.map(column => column.low)
    .slice(0, DAYS - 1 + QUOTES)
    .reverse();
  const closePrices = quotes.map(column => column.close)
    .slice(0, DAYS - 1 + QUOTES)
    .reverse();
  const volumes = quotes.map(column => column.volume)
    .slice(0, DAYS - 1 + QUOTES)
    .reverse();
